...
/**
 * Implementation V1p3LtiToolProviderProvisionSettingsForm class.
 *
 * @package Drupal\lti_tool_provider_provision\Form
 */
class V1p3LtiToolProviderProvisionSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $filter = ''): array {
    ...
    $lti_roles = $this->config('lti_tool_provider.settings')->get('v1p3_lti_roles');

    $entityType = $form_state->getValue('entity_type') ? $form_state->getValue('entity_type') : $settings->get('v1p3_entity_type');
    $entityBundle = $form_state->getValue('entity_bundle') ? $form_state->getValue('entity_bundle') : $settings->get('v1p3_entity_bundle');
    $entityRedirect = $form_state->getValue('entity_redirect') ? $form_state->getValue('entity_redirect') : $settings->get('v1p3_entity_redirect');
    $entityDefaults = $form_state->getValue('entity_defaults') ? $form_state->getValue('entity_defaults') : $settings->get('v1p3_entity_defaults');
    ...
    foreach ($lti_roles as $ltiRole) {
      $form['allowed_roles'][$ltiRole] = [
        ...
        '#default_value' => $allowedRoles[encode_key($ltiRole)],
      ];
    }
  ...
  public function getFormId(): string {
    return 'v1p3_lti_tool_provider_provision_settings';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $settings->set('v1p3_entity_redirect', $entityRedirect);
    $settings->set('v1p3_entity_sync', $entitySync);
    $settings->set('v1p3_allowed_roles_enabled', $allowedRolesEnabled);
    ...
    $settings->set('v1p3_entity_defaults', $entityDefaults);

    $allowedRoles = [];
    foreach ($form_state->getValue('allowed_roles') as $key => $value) {
      $allowedRoles[encode_key($key)] = $value;
    }
